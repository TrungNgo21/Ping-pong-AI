...
class Score(Ball, Bar):
    ...
    def draw_score(self):
        ...
        if Score.right_score > 4:
            ...
            Score.win.blit(winning_text_right, (
                WIDTH // 2 - winning_text_right.get_width() // 2, HEIGHT // 2 - winning_text_right.get_height() // 2))
            ...
            Score.win.blit(winning_text_left, (
                WIDTH // 2 - winning_text_left.get_width() // 2, HEIGHT // 2 - winning_text_left.get_height() // 2))
            pygame.display.update()
            pygame.time.delay(5000)
            ball.reset_after_one_game()
            left_bar.reset()
...
def move_of_bar(keys, left_bar, right_bar):
    move = database.child("Motion").child("Status").get().val()
    print(move)
    if move == '0':
